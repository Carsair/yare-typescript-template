...
const Merge = {
  ...
  mergeTogetherStrategy: (spirit: Spirit) => {
    ...
    if (spirit.size > 1) console.log('MERGE: should split?, size, mark, divide: ', spirit.energy/spirit.energy_capacity < .9, spirit.size, spirit.mark, !!spirit.divide);
    if (spirit.size > 1 && (spirit.mark == "empty" || spirit.energy/spirit.energy_capacity < .5)) {
      // spirit.shout(`dying divide`)
      return spirit.divide && spirit.divide()
    }
    const maxCapac = 16 //10 * Consts.enemyAliveSpirits[0].energy_capacity;
    if (spirit.sight.friends.length > 0) {
      ...
      if (closestFriendToMe) {
        if (spirit.size + closestFriendToMe.size <= maxCapac && (spirit.energy + closestFriendToMe.energy) / (spirit.energy_capacity + closestFriendToMe.energy_capacity) > .95) {
